...
export class Logger {
    ...
    ////////////////////////////////////////////////////////////
    constructor({ req, room }: { req?: Request, room?: Room }) {
        if (req) {
            this.req = req;
    ...
    error(message: string | Error, options?: LogOptions): void {
        ...
        // If the error occurred from room
        if (this.room) {
            options = {
                ...options,
                ...
                data: {
                    ...(options?.data || {}),
                    participants: Object.keys(this.room.participants),
                },
            };

            if (options.sender) {
                const participant = this.room.participants[options.sender];
                options.data.capabilities = participant.capabilities;
                options.data.producers = participant.producers;
                options.data.consumers = participant.consumers;
                options.data.transports = participant.transports;
            }
        }
